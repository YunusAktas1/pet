...
TEST_DB_FILENAME = "test_match_decision.db"
...
def test_decide_and_filter_matches(client: TestClient) -> None:
    ...
    response = client.post(
        ...
        json={"name": "TomX", "species": "cat", "gender": "male"},
    )
    assert response.status_code == 200, response.text
    ...
    assert response.status_code == 200, response.text
    data = response.json()
    assert data["target_pet_id"] == pet_id
